    ...
    public class GraphController : ControllerBase
    {
        ...
        public IActionResult Command([FromQuery] string query)
        {
            try
            {
                ...
                {
                    return BadRequest("No database is currently loaded. Please create or load a database.");
                }
                var result = GraphManager.CurrentGraph.ExecuteCypherCommand(query); // Use ExecuteCypherCommand for Cypher-style queries
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        //[HttpPost("load")]
        //public IActionResult LoadGraph([FromBody] string dbName)
        //{
        //    if (string.IsNullOrWhiteSpace(dbName))
        //        return BadRequest("Database name must be provided.");

        //    GraphManager.LoadGraph(dbName);
        //    if (GraphManager.IsDatabaseLoaded)
        //        return Ok($"Database '{dbName}' loaded successfully.");
        //    else
        //        return NotFound($"Database '{dbName}' could not be loaded.");
